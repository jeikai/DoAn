    ...
    <div class="flex border shadow-md">
      <div class="w-1/3 border-r">
        <div class="header flex gap-4 items-center h-14 border-b px-4 mb-4">
          ...
          <UInput icon="i-heroicons-magnifying-glass-20-solid" size="sm" color="white" :trailing="false" name="input"
            placeholder="Search..." />
        ...
        <div class="px-2 w-full h-[75vh] overflow-x-auto">
          <div class="flex gap-4 p-3 mb-2 rounded-md" v-for="chat in chatGroup">
            ...
            <div class="flex-1" @click="selectChat(chat.id)">
      ...
      <div class="w-2/3 w-full">
        <div>
          <div v-for="chat in chatGroup" v-if="chatId">
            <ChatLayout :chat-id="chatId" v-if="chatId == chat.id" />
          </div>
          <div class="welcome_container flex-container" v-else>
...
const { chatId: propChatId } = defineProps(['chatId']);
const chatId = ref(propChatId);
...
async function loadData() {
  try {
    ...
    studentOptions.value = responsePeople.data.userData.map((person) => ({
      _id: person._id,
      email: person.email,
      name: person.name,
    }));
