<template>
  <div class="header flex gap-4 items-center h-14 border-b px-4">
    ...
    <div>
      ...
      <Icon name="material-symbols:person-add" class="mr-4 mt-[2px] text-gray-500" size="25" @click="isOpenAdd = true" />
        ...
        <UForm :schema="schema" :state="state" @submit="submit">
          <UFormGroup class="mb-4 flex-1" label="Add by email" name="listUser">
            <Select v-model:value="state.listUser"
              :options="filteredUserData.map(user => ({ label: user.email, value: user._id }))" mode="tags"
