    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <Select
              v-model:value="state.listUser"
              :options="
                studentOptions.map((t) => ({ label: t.email, value: t._id }))
              "
              mode="tags"
          ...
          >
            <Select
              v-model:value="state.examinationBoard"
              :options="
                examination.map((t) => ({ label: t.name, value: t._id }))
              "
              mode="tags"
              placeholder="Please select"
              class="w-100"
...
<script setup lang="ts">
  ...
  async function loadData() {
    try {
      ...
      const response_people = await axios.get(
  ...
  async function submit(event: FormSubmitEvent<Schema>) {
    ...
    if (
      ...
      !event.data.teacherId ||
      event.data.listUser.length === 0
    ) {
      ...
      try {
        const updatedListUser = [
          ...event.data.listUser,
          event.data.teacherId,
          ...event.data.examinationBoard,
        ];

        const response = await axios.post(
          'http://localhost:5000/api/class',
          { ...event.data, listUser: [...event.data.listUser] },
          { headers }
        );
        const newChat = {
